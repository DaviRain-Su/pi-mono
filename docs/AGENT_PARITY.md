...
| `src/harness/session/repository.ts` | `agent/harness/session/repo/shared.zig` | partial | Pure `createSessionForkSelection` + `readSessionEntriesForFork` + session id/timestamp helpers; full SessionRepository interface still missing |
| `src/harness/session/jsonl-repo.ts` | `session/repo/jsonl.zig` | missing | Stub only |
| `src/harness/session/memory-repo.ts` | `session/repo/memory.zig` | partial | `InMemorySessionBackend` + `InMemorySessionStorage` projections via `ArraySessionIndex` + `KeyedOperationQueue`; dispose rejection; offline tests on backend/storage (`appendEntry`). `InMemorySessionRepository`/`createSession` facade still stretch |
...
| Bounded branch queries (`branch-query`) | `session/array_session_index.zig` + memory storage | partial | Array index readers implemented; memory storage wires `findEntriesOnBranch`/`readPathToRootOrCompaction`; jsonl compose still missing |
...
2. ~~Wire `ArraySessionIndex` into memory session storage/backend~~ done (jsonl still open)
